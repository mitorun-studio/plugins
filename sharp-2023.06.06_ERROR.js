...
const sharpWebpOptions = {
	...
	effort: 6,// ресурсы процессора, "0-6", default "4".
};

const sharpAvifOptions = {
	quality: 50,// качество "1-100", default "50".
	//lossless: true, сжатие без потерь, default "false".
	effort: 9,// ресурсы процессора, "0-9", default "4".
};
...
function processDirectory(inputDir, outputDir) {
	...
	filenames.forEach((file) => {
		...
		const outputPath = path.join(outputDir, path.basename(file, path.extname(file)) + '.' + outputExtension);
